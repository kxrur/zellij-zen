...
Download the prebuilt plugin and load it from a keybind or at startup.
...
The first time it runs, Zellij will ask you to grant two permissions:

- `ReadApplicationState` — to inspect the current panes
- `ChangeApplicationState` — to hide/show the bar panes

Answer `y` once; the grant is cached for future runs.
